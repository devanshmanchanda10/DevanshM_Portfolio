      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        <div className="lg:flex lg:justify-between lg:gap-20">
          ...
          <header className="lg:w-[40%] lg:sticky lg:top-0 lg:h-screen lg:py-24 py-12 flex flex-col justify-between">
            <div>
              <div className="font-display font-bold text-xl tracking-tight text-slate-900 mb-8">
                DM<span className="text-blue-600">.</span>
              </div>

              <h1 className="font-display text-5xl md:text-6xl font-bold text-slate-900 leading-[1.1] mb-6 tracking-tight">
                Devansh <br /> Manchanda
              </h1>

              <div className="inline-flex items-center px-3 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-600 text-sm font-bold tracking-wide uppercase mb-8">
              ...
              <nav className="hidden lg:block mb-12">
                <ul className="space-y-4">
                  {['overview', 'projects', 'skills', 'education'].map((item) => (
                    <li key={item}>
                      <button
                        onClick={() => scrollToSection(item === 'overview' ? 'experience' : item)} // Overview maps to top of right col (Experience)
                        className={`group flex items-center text-sm font-bold uppercase tracking-widest transition-all ${activeSection === item || (item === 'overview' && activeSection === 'experience')
                          ? 'text-slate-900'
                          : 'text-slate-400 hover:text-slate-600'
                          }`}
                      >
                        <span className={`mr-4 h-px transition-all ${activeSection === item || (item === 'overview' && activeSection === 'experience') ? 'w-16 bg-slate-900' : 'w-8 bg-slate-300 group-hover:w-12 group-hover:bg-slate-600'}`}></span>
                        {item}
                      </button>
                    </li>
                  ))}
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="px-6 py-3 bg-slate-900 text-white text-sm font-semibold rounded-xl hover:bg-blue-600 transition-colors shadow-lg shadow-slate-900/20"
              >
                Let's Talk
              </button>
            </div>
